...
void msImageProcessor::DefineImage(byte *data_, imageType type, int height_,
                                   int width_) {
...
void msImageProcessor::DefineBgImage(byte *data_, imageType type, int height_,
                                     int width_) {
...
void msImageProcessor::GetRawData(float *outputImageData) {
  ...
  if (!outputImageData) {
    ErrorHandler("msImageProcessor", "GetRawData",
...
void msImageProcessor::GetResults(byte *outputImage) {
  ...
  if (!outputImage) {
    ErrorHandler("msImageProcessor", "GetResults",
...
int msImageProcessor::GetRegions(int **labels_out, float **modes_out,
                                 int **MPC_out) {
  ...
  // done. Return the number of regions resulting from filtering or
  // segmentation.
  return regionCount;
}
...
void msImageProcessor::NonOptimizedFilter(float sigmaS, float sigmaR) {
  ...
  // been defined...
  if (!height) {
    ErrorHandler("msImageProcessor", "LFilter",
  ...
  if (((h[0] = sigmaS) <= 0) || ((h[1] = sigmaR) <= 0)) {
    ErrorHandler("msImageProcessor", "Segment",
...
void msImageProcessor::OptimizedFilter1(float sigmaS, float sigmaR) {
  ...
  // Declare Variables
  int iterationCount, i, j, k, s, p, modeCandidateX, modeCandidateY,
      modeCandidate_i;
    ...
    for (i = 0; i < 8; i++) {
      ...
      /*
                     ...
                     if((regionLoc%width == 0)&&((i == 3)||(i == 4)||(i == 5)))
                             continue;
                     if((regionLoc%(width-1) == 0)&&((i == 0)||(i == 1)||(i ==
         7))) continue;
      */
      ...
      if ((neighLoc >= 0) && (neighLoc < imageSize) && (labels[neighLoc] < 0)) {
        for (k = 0; k < N; k++) {
          //					if(LUV_data[(regionLoc*N)+k] !=
          //LUV_data[(neighLoc*N)+k])
          if (fabs(LUV_data[(regionLoc * N) + k] -
                   LUV_data[(neighLoc * N) + k]) >= LUV_treshold)
            break;
        }
...
void msImageProcessor::BuildRAM(void) {
  ...
  // Allocate memory for region adjacency matrix if it hasn't already been
  // allocated
...
void msImageProcessor::TransitiveClosure(void) {
  ...
  //   canonical element of that region (e.g. raList[i], initially has
  //   raList[i].label = i, namely each region is initialized to have itself as
    ...
    while (curRegion) {
      ...
      if (curLabel > x) {
        ...
        if ((edgePixelCount = curRegion->edgePixelCount +
                              neighborRegion->edgePixelCount) != 0) {
          ...
          curRegion->edgePixelCount = neighborRegion->edgePixelCount =
              edgePixelCount;
        }
      }
...
void msImageProcessor::Prune(int minRegion) {
  ...
  // Declare variables
  int i, k, candidate, iCanEl, neighCanEl, iMPC, label, oldRegionCount,
      minRegionCount;
  double minSqDistance, neighborDistance;
  RAList *neighbor;

  // Apply pruning algorithm to classification structure, removing all regions
  // whose area is under the threshold area minRegion (pixels)
  do {
    ...
    // Traverse the RAM joining regions whose area is less than minRegion
    // (pixels) with its respective candidate region.

    // A candidate region is a region that displays the following properties:

    //	- it is adjacent to the region being pruned

    //  - the distance of its mode is a minimum to that of the region being
    //  pruned
    //    such that or it is the only adjacent region having an area greater
    //    than minRegion
    ...
    for (i = 0; i < regionCount; i++) {
      ...
      // Note: Adjust this if statement if a more sophisticated pruning
      // criterion
      //       is desired. Basically in this step a region whose area is less
      //       than minRegion is pruned by joining it with its "closest"
      //       neighbor (in color). Therefore, by placing a different criterion
      //       for fusing a region the pruning method may be altered to
      //       implement a more sophisticated algorithm.
...
void msImageProcessor::DefineBoundaries(void) {
  ...
  for (i = 1; i < height - 1; i++) {
    ...
    for (j = 1; j < width - 1; j++) {
      ...
      if ((label != labels[dataPoint - 1]) ||
          (label != labels[dataPoint + 1]) ||
  ...
  int *boundaryBuffer = new int[totalBoundaryCount],
      *boundaryIndex = new int[regionCount];
...
bool msImageProcessor::InWindow(int mode1, int mode2) {
  ...
  while ((diff < 0.25) && (k != kp)) // Partial Distortion Search
  {
    ...
    diff = 0;
    for (p = 0; p < P[k]; p++) {
      el = (modes[mode1 * N + p + s] - modes[mode2 * N + p + s]) /
           (h[k] * offset[k]);
...
void msImageProcessor::InitializeOutput(void) {
  ...
  // Allocate memory used to store image modes and their corresponding
  // regions...
...
void msImageProcessor::NewOptimizedFilter1(float sigmaS, float sigmaR) {
  ...
  for (i = 0; i < L; i++) {
    ...
    if (N == 3)
      mvAbs +=
          (Mh[2] * Mh[2] + Mh[3] * Mh[3] + Mh[4] * Mh[4]) * sigmaR * sigmaR;
    else
