...
const double EPSILON =
    0.01; // define threshold (approx. Value of Mh at a peak or plateau)
const double MU =
    0.05; // define threshold required that window is near convergence
const double TC_DIST_FACTOR =
    0.5; // cluster search windows near convergence that are a distance
...
protected:
  ...
  void MSVector(double *, double *); // Computes the mean shift vector at a
                                     // specified window location yk in the data
                                     // set x given the vector yk
  /*/\/\/\/\/\/\/\/\/\/\/\/\/\/\*/
  ...
  void ErrorHandler(const char *, const char *,
                    const char *); // flags an error and halts the system
  ...
  // commented out -- STB 7/16/05
  //	msSystem		msSys;
  //// used for function timing and system output
  ...
  int L, N, kp,
      *P; // length, dimension, subspace number, and subspace dimensions
  ...
  float *offset; // defines bandwidth offset caused by the use of a Gaussian
                 // kernel (for example)
  ...
  unsigned char
      *modeTable; // Assigns a marking to each data point specifying whether
                  // or not it has been assigned a mode. These labels are:
                  // modeTable[i] = 0 - data point i is not associated with a
                  // mode modeTable[i] = 1 - data point i is associated with a
                  // mode modeTable[i] = 2 - data point i is associated with a
                  // mode
  ...
  ClassStateStruct
      class_state; // specifies the state of the class(i.e if data has been
                   // loaded into the class, if a kernel has been defined, etc.)
...
private:
  ...
  void ResetInput(void); // de-allocate memory for and re-initialize input data
                         // structure and mode structure
  ...
  ////////Data Search Tree/////////
  tree *
  BuildKDTree(tree *, int, int,
              tree *); // Builds a kd tree given a subset of points initialized
                       // at depth 0 (dimension 0) (for Tree Structure)

  void
  QuickMedian(tree *, int, int,
              int); // Finds the median tree in a forest of trees using
                    // dimension d, placing the median tree in the array of tree
                    // nodes at L/2, in which all trees to the left of the
                    // median tree have values less than that of the median tree
                    // in dimension d and all trees having values greater than
                    // that of the median tree in dimension d are placd to the
                    // right of this tree - This algorithm is used by
                    // BuildKDTree to construct a balanced tree
  ...
  void uniformSearch(tree *, int, double *,
                     double *); // uses kdbst to perform range search on input
                                // data, computing the weighted sum of these
                                // points using a uniform kernel and storing the
                                // result into Mh (called by uniformMSVector)

  void generalSearch(tree *, int, double *,
                     double *); // uses kdbst to perform range search on input
                                // data, computing the weighted sum of these
                                // points using a general kernel and storing the
                                // result into Mh (called by generalMSVector)
  ...
  void
  uniformLSearch(double *,
                 double *); // given a center location and mean shift vector, a
                            // lattice search is performed to compute the mean
                            // shift vector using a uniform kernel

  void optUniformLSearch(
      double *, double *); // given a center location and mean shift vector, a
                           // lattice search is performed to compute the mean
                           // shift vector using a uniform kernel and the basin
                           // of attraction optimization for better performance

  void
  generalLSearch(double *,
                 double *); // given a center location and mean shift vector, a
                            // lattice search is performed to compute the mean
                            // shift vector using a general kernel

  void optGeneralLSearch(
      double *, double *); // given a center location and mean shift vector, a
                           // lattice search is performed to compute the mean
                           // shift vector using a general kernel and the basin
                           // of attraction optimization for better performance
  ...
  ////////Lattice Data Structure////////
  int LowerBoundX, UpperBoundX; // Upper and lower bounds for lattice search
                                // window in the x dimension

  int LowerBoundY, UpperBoundY; // Upper and lower bounds for lattice search
